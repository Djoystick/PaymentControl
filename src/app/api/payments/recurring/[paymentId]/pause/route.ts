...
export async function POST(request: Request, context: RouteContext) {
  ...
  let body: PauseSubscriptionBody = {};
  try {
    body = (await request.json()) as PauseSubscriptionBody;
  } catch {
    body = {};
  }
  ...
  const pauseResult = await setSubscriptionPausedStateForPayment(
  ...
  if (!pauseResult.ok) {
    if (pauseResult.reason === "NOT_FOUND") {
      return jsonError("PAYMENT_NOT_FOUND", "Recurring payment was not found.");
    }

    if (pauseResult.reason === "PAYMENT_ARCHIVED") {
      return jsonError(
        "PAYMENT_VALIDATION_FAILED",
        "Archived payment cannot be paused.",
      );
    }

    if (pauseResult.reason === "NOT_SUBSCRIPTION") {
      return jsonError(
        "PAYMENT_VALIDATION_FAILED",
        "Only subscription payments can be paused.",
      );
    }

    return jsonError("PAYMENT_UPDATE_FAILED", "Failed to pause subscription.");
  }

  return NextResponse.json<PaymentMutateResponse>({
    ok: true,
    payment: pauseResult.payment,

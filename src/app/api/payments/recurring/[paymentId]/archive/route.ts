...
import { archiveRecurringPaymentForWorkspace } from "@/lib/payments/repository";

type ArchivePaymentBody = {
  initData?: string;
};
...
export async function POST(request: Request, context: RouteContext) {
  ...
  let body: ArchivePaymentBody = {};
  try {
    body = (await request.json()) as ArchivePaymentBody;
  } catch {
    body = {};
  }
  ...
  const archived = await archiveRecurringPaymentForWorkspace(
  ...
  if (!archived) {
    return jsonError(
      ...
      "Failed to archive recurring payment for current workspace.",
    );
  }

...
import { readPaymentsDashboardByWorkspace } from "@/lib/payments/repository";

type ReadDashboardBody = {
  initData?: string;
};
...
const jsonError = (
  ...
  return NextResponse.json<PaymentsDashboardResponse>(
    {
      ok: false,
      error: { code, message },
    },
...
export async function POST(request: Request) {
  ...
  let body: ReadDashboardBody = {};
  try {
    body = (await request.json()) as ReadDashboardBody;
  } catch {
    body = {};
  }
  ...
  const dashboard = await readPaymentsDashboardByWorkspace(
  ...
  if (!dashboard) {
    return jsonError(
      "PAYMENT_LIST_FAILED",
      "Unable to load dashboard summary for current workspace.",
    );
  }
